...
# Formula : z = (x1 * w1 + x2 * w2 + x3 * w3 ... + xn * wn) + b
# Parameters :
...
#------------------------------------------------------------
# Function Name : Marvellous Display Multiplication Details
...
def Marvellous_Process_Hidden_Layer(inputs, hidden_weights, hidden_biases):
    ...
    for neuron_index in range(len(hidden_weights)):
        ...
        print("Step 3 : Apply ReLU activation")
...
# Description : Processes output layer neuron using Sigmoid activation function
# Parameters :
...
def Marvellous_ANN_Forward_Pass(inputs):
    ...
    #------------------------------------------------------------
    hidden_weights = [
        [0.5, -0.2],    # Weights of hidden neuron 1
        [0.8, 0.4]      # Weights of hidden neuron 2   
    ]
    ...
    z_output, final_output = Marvellous_Process_Output_Layer(
        hidden_outputs, output_weights, output_bias
    )
