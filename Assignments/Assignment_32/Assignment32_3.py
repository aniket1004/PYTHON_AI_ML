...
loggerObj = FileLogger("Assignment32_3.log")

def DirectoryFileDuplicateDeletion(DirectoryName):
    ...
    try:
        ...
        loggerObj.WriteLine(f"\nTotal Deleted Files From {DirectoryName} is : {DeletedFileCount}")
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        if ((sys.argv[1] == "--h") or (sys.argv[1] == "--H")):
            print("This automation script accept directory name. Delete all duplicate files from that directory and write names of duplicate files from that directory into log file named as Log.txt. Log.txt file should be created into current directory.")
            ...
            DirectoryFileDuplicateDeletion(DirectoryName=DirectoryName)
    else:
