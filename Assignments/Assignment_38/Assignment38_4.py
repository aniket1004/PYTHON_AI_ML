...
def main():
    ...
    final_result_col = dataframe["FinalResult"].value_counts()
    pass_percen = (final_result_col[1]/ (final_result_col[0] + final_result_col[1])) * 100
    fail_percen = (final_result_col[0]/ (final_result_col[0] + final_result_col[1])) * 100
    
    print(f"Percentage passed students : {pass_percen}%")       # 60.0 %
    print("Percentage failed students : %2.f%%" % fail_percen)  # 40.0 %
    # Balanced is 50-50 but 60-40 is also good
    # if percentage is 20% or less in binary label result then it is problem
    
    plt.figure(figsize=(8,6))
    sns.countplot(x = dataframe["FinalResult"])
    plt.title("Distribution of Final Result to find dataset is balanced.")
    plt.show()
