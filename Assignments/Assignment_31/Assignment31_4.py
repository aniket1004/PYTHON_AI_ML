...
loggerObj = FileLogger("Assignment31_4.log")

def DirectoryFileCopyToAnotherDirectoryWithExtension(DirectoryName,  NewDirectory, Extension):
    global loggerObj
    ...
    try:
        Count = 0
        UpdatedExtension = Extension.replace(".", "")
        ...
        for Folders, SubFolders, FileNames in os.walk(DirectoryName):
            ...
            for FileName in FileNames:
                ...
                if os.path.isfile(Fname):
                    try:
                        FileExtension = FileOperation.GetFileExtension(Fname)
                        if FileExtension == UpdatedExtension:
        ...
        loggerObj.WriteLine(f"{Count} Files successfully copied from directory {DirectoryName} to {NewDirectory} with extension {Extension}")
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        if ((sys.argv[1] == "--h") or (sys.argv[1] == "--H")):
            print("This automation script  accept two directory names. Copy all files with the specified extension from first directory into second directory.")
        ...
        DirectoryFileCopyToAnotherDirectoryWithExtension(DirectoryName=DirectoryName, NewDirectory=NewDirectoryName, Extension=Extension)
