...
def main():
    ...
    for i in range(len(X.columns)):
        print(f"Feature {X.columns[i]} importance score is : {model.feature_importances_[i]}")

    feature_importanaces = pd.Series(model.feature_importances_,index=X.columns)
    feature_importanaces.plot(kind="barh")
    plt.show()

    # Study Hours Feature showing highest value 1
    # Other all feature showing lowest value 0
